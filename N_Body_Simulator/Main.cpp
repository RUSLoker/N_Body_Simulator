...
#define point_b(i, j) point_b[i*2 + j]
    ...
    while (window.isOpen())
    {
        ...
        memcpy(point_b, points, sizeof(double) * config.N * 2);
        ...
        window.setTitle(to_string(fps) + " / " + to_string(ups) + " / " + to_string(ups * config.DeltaT) + " / " + to_string(depth) + " / " + to_string(totalNodes) + " / " + to_string(activeNodes));
    }
}